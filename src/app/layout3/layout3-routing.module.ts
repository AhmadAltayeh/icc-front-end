...
const routes: Routes = [
  {
    ...
    children: [
      { path: '', pathMatch: 'full', redirectTo: 'my-courses' },
      {
        path: 'my-courses',
        loadChildren: () =>
          import('../pages3/my-courses/my-courses.module').then(
            (m) => m.MyCoursesModule
          ),
      },
      {
        path: 'courses',
        loadChildren: () =>
          import('../pages3/courses/courses.module').then(
            (m) => m.CoursesModule
          ),
      },
      {
        path: 'profile',
        loadChildren: () =>
          import('../pages3/profile/profile.module').then(
            (m) => m.ProfileModule
          ),
      },
      {
        path: '404',
        loadChildren: () =>
          import('../pages3/not-found/not-found.module').then(
            (m) => m.NotFoundModule
          ),
      },
      {
